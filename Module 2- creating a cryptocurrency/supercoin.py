...
def is_valid():
    ...
    if is_valid:
        ...
        response = {'message': 'Vakul There is some error !!! , The Blockchain in not valid \\'}
    return jsonify(response), 200
...
def connect_node():
    ...
    response = {'message': 'All nodes are now connected. The Supercoin Blockchain now contains the following nodes',
...
def replace_chain():
    ...
    if is_chain_replaced:
        response = {'message': 'The Nodes have different chains so the chain is replaced',
                    'new_chain': blockchain.chain}
    else:
        response = {'message': 'All Good !! The chaon is the longest One \\',
                    'actual_chain': blockchain.chain}
    return jsonify(response), 200


# running the app
app.run(host='0.0.0.0',port = 5000)  #  change 5000 with some other number to create different nodes